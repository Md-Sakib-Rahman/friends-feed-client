...
import { 
  Users, UserMinus, UserCheck, X, 
  MessageSquare, Zap, Clock, Send, Search, ChevronRight 
} from "lucide-react";
...
const FriendsPage = () => {
  ...
  const handleResponse = async (requestId, status) => {
    try {
      ...
      toast.success(status === 'accepted' ? 'Added to circle' : 'Request declined');
      fetchData();  
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto px-6 pt-12 relative z-10">
        ...
        <section className="bg-base-200/30 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-4 mb-10 shadow-xl shadow-black/5">
          <div className="px-4 py-2 mb-2 border-b border-white/5">
             <span className="text-[10px] font-black opacity-30 uppercase tracking-widest">Active Connections</span>
          </div>
            ...
            {filteredFriends.length > 0 ? (
              filteredFriends.map((friend) => (
                ...
                >
                  ...
                  {/* Name & Username */}
                  <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate(`/social/user/${friend._id}`)}>
                    <h4 className="font-bold text-sm truncate group-hover:text-primary transition-colors">{friend.name}</h4>
                    <p className="text-[10px] opacity-40 font-black uppercase tracking-tighter truncate">@{friend.username}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                    ...
                    <button onClick={() => handleUnfriendClick(friend)}  className="btn btn-ghost btn-sm p-2 hover:bg-error/10 hover:text-error rounded-xl">
                      <UserMinus size={18} />
                    </button>
                  </div>
                </motion.div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {(activeTab === "incoming" ? incomingRequests : sentRequests).length > 0 ? (
                  (activeTab === "incoming" ? incomingRequests : sentRequests).map((req) => (
                    <div key={req._id} className="flex items-center justify-between p-3 bg-white/5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
                      <div className="flex items-center gap-3">
                        <img 
                          src={(activeTab === 'incoming' ? req.sender : req.receiver)?.profilePicture} 
                          className="w-10 h-10 rounded-lg object-cover flex-shrink-0" 
                          alt="" 
                        />
                        <div>
                          <h5 className="font-bold text-xs">{(activeTab === 'incoming' ? req.sender : req.receiver)?.name}</h5>
                          <p className="text-[9px] opacity-40 font-black uppercase tracking-tighter">
                            {activeTab === 'incoming' ? 'Wants to connect' : 'Pending response'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        {activeTab === 'incoming' ? (
                          <>
                            <button onClick={() => handleResponse(req._id, 'accepted')} className="btn btn-primary btn-xs rounded-lg font-bold px-3">Accept</button>
                            <button onClick={() => handleResponse(req._id, 'rejected')} className="btn btn-ghost btn-xs rounded-lg px-3">Ignore</button>
                          </>
                        ) : (
                          <button onClick={() => handleCancelRequest(req._id)} className="btn btn-ghost btn-xs text-error font-bold hover:bg-error/10 px-3">Withdraw</button>
                        )}
                      </div>
                    </div>
                  ))
  ...
  {unfriendTarget && (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      >
        ...
        <h3 className="text-2xl font-black tracking-tight mb-2">Break Connection?</h3>
        <p className="text-sm opacity-60 font-medium mb-8 leading-relaxed">
          Are you sure you want to remove <span className="text-base-content font-bold">@{unfriendTarget.username}</span> from your circle?
        </p>
