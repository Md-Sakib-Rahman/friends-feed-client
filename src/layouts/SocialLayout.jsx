...
const SocialLayout = () => {
  ...
  const { unreadCount, onlineUsers, msgUnreadCount } = useSocket();
  ...
  const activeFriends = friends.filter((f) => onlineUsers.includes(f._id));
            ...
            <div className="relative p-[1px] h-full rounded-[2.5rem] overflow-hidden group">
              ...
              <div className="relative bg-base-100/60 backdrop-blur-3xl rounded-[2.5rem] p-6 flex flex-col h-full">
                ...
                <div className="mt-auto space-y-4 pt-4 border-t border-base-content/5">
                  ...
                  <Link
                    to="/social/profile"
                    className="flex items-center gap-3 p-2 rounded-2xl hover:bg-base-200/50 transition-colors"
                  >
          ...
          <main className="flex-1 max-w-2xl mx-auto w-full pb-32 lg:pb-0">
            ...
            <div className="lg:hidden flex items-center justify-between mb-6 bg-base-100/40 backdrop-blur-2xl p-4 rounded-3xl border border-white/5 shadow-xl fixed top-4 left-1/2 -translate-x-1/2 w-[90%] z-50">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  to="/social/profile"
                  className="w-9 h-9 rounded-full ring-2 ring-primary/20 overflow-hidden"
                >
                  <img
          ...
          <aside className="hidden xl:flex flex-col w-80 sticky top-6 h-[calc(100vh-48px)]">
            <div className="relative p-[1px] h-full rounded-[2.5rem] overflow-hidden group">
              ...
              <div className="relative bg-base-100/60 backdrop-blur-3xl rounded-[2.5rem] p-6 h-full flex flex-col">
                <p className="text-[10px] font-black uppercase tracking-widest opacity-40 mb-6 px-2 italic">
                  Active Friends
                </p>
                <div className="space-y-4">
                  {activeFriends.length > 0 ? (
                    activeFriends.map((friend) => (
                      ...
                      >
                        <div className="relative flex-shrink-0">
                          ...
                          <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-[3px] border-base-100 shadow-lg">
                            <div className="w-full h-full bg-green-400 rounded-full animate-ping opacity-75"></div>
                          </div>
                        </div>

                        <div className="flex-1 min-w-0">
                          <p className="font-black text-[13px] truncate group-hover/friend:text-primary transition-colors">
                            {friend.name}
                          </p>
                    ...
                    <div className="text-center py-10 bg-base-200/20 rounded-[2.5rem] border border-dashed border-white/5">
                      <Zap size={24} className="mx-auto mb-2 opacity-10" />
                      <p className="text-[10px] font-bold uppercase opacity-20 tracking-widest">
                        No friends active
                      </p>
                    </div>
                  )}
                ...
                <Link
                  to="/social/friends"
                  className="btn btn-ghost btn-sm mt-auto text-primary rounded-xl font-bold"
                >
                  Manage Connections
                </Link>
              </div>
